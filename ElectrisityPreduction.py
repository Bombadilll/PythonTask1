...
# read file
data = pd.read_excel("data.xlsx",sheet_name='SheetAuth2020')
...
#export predict to exel file
dataPredict=pd.DataFrame(data_predict)
with ExcelWriter("DataPredictScaler.xlsx") as writer:
    dataPredict.to_excel(writer,sheet_name="Sheet1")
...
#export predict to exel file
dataPredict1 = pd.DataFrame(data_predict)
with ExcelWriter("DataPredictScaler.xlsx",mode="a") as writer:
    dataPredict1.to_excel(writer,sheet_name="Sheet2")
...
plt.plot(dates, kwhs)
plt.show()
#export forecast to exel file
dataPredict=pd.DataFrame( kwhs,dates,)
dataPredict.to_excel("ForecastLoadDay.xlsx","Sheet1")
print("ForecastLoadDay.xlsx saved.")
with ExcelWriter("DataPredictScaler.xlsx",mode="a") as writer:
    dataPredict.to_excel(writer,sheet_name="Sheet3")
